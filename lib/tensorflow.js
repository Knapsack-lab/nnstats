...
function analyzeConvTF(input, filter, strides, padding, options) {
	...
	strides = errorCheck.check4Dstrides(strides);
	...
	if (padding === 'VALID') {
		width = Math.ceil((input[0] - filter[0] + 1) / strides[1]);
		height = Math.ceil((input[1] - filter[1] + 1) / strides[2]);
	} else {
		width = Math.ceil(input[0] / strides[1]);
		height = Math.ceil(input[1] / strides[2]);
	}
...
function analyzePoolTF(input, filter, strides, padding, options) {
	...
	if (padding === 'VALID') {
		...
		let padAlongHeight = Math.max((height - 1) * strides[1] + filter[0] - input[0], 0);
		let padAlongWidth = Math.max((width - 1) * strides[2] + filter[1] - input[1], 0);
 ...
 * @return {Object}	statistics of the convolution layer
 */
function getPaddings(input, filter, strides) {
	input = errorCheck.check2Or3DInput(input);
	filter = errorCheck.check2DFilter(filter);
	strides = errorCheck.check4Dstrides(strides);

	let outWidth = Math.ceil(input[0] / strides[1]);
	let outHeight = Math.ceil(input[1] / strides[2]);

	let padAlongHeight = Math.max((outHeight - 1) * strides[1] + filter[0] - input[0], 0);
	let padAlongWidth = Math.max((outWidth - 1) * strides[2] + filter[1] - input[1], 0);
