...
function analyzeConvTF(input, filter, stride, padding, options) {
	...
	if (padding === 'VALID') {
		width = Math.ceil((input[0] - filter[0] + 1) / stride[0]);
		height = Math.ceil((input[1] - filter[1] + 1) / stride[1]);
	} else {
		width = Math.ceil(input[0] / stride[0]);
		height = Math.ceil(input[1] / stride[1]);
	}
...
function analyzePoolTF(input, filter, stride, padding, options) {
	...
	if (padding === 'VALID') {
		...
		let padAlongHeight = Math.max((height - 1) * stride[0] + filter[0] - input[0], 0);
		let padAlongWidth = Math.max((width - 1) * stride[1] + filter[1] - input[1], 0);
...
function getPaddings(input, filter, stride) {
	...
	let outWidth = Math.ceil(input[0] / stride[0]);
	let outHeight = Math.ceil(input[1] / stride[1]);

	let padAlongHeight = Math.max((outHeight - 1) * stride[0] + filter[0] - input[0], 0);
	let padAlongWidth = Math.max((outWidth - 1) * stride[1] + filter[1] - input[1], 0);
