...
function analyzeFC(input, hidden, options) {
	...
	if (input.constructor === Array) {
		...
		let reshape = [input[0] * input[1] * input[2]];

		if (options.batchSize)
			reshape.splice(0, 0, options.batchSize);
	...
	return {
		output, connectionsPerNeuron, neurons, weights, extras
	}
}
...
function analyzeNetwork(networkJson, input, options) {
	...
	for (var i = 0; i < networkJson.length; i++) {
		...
		if (layer.datasize > 0) {
			...
			stat.connMemory = layer.datasize * stat.weights;
			stat.reducedConnMemory = layer.datasize * (stat.reducedWeights || stat.weights);
			memory += stat.memory;
		}
