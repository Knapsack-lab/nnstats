 ...
 * let stride = [1, 1];
 * let padding = [0, 0];
 * let stats = analyzer.analyzeConv(input, filter, stride, padding);
 */
function analyzeConv(input, filter, stride, padding, options) {
	options = options || {};
	padding = padding || [0, 0];

	input = errorCheck.check2Or3DInput(input);
	filter = errorCheck.check3DFilter(filter);
	stride = errorCheck.check2DStride(stride);
	padding = errorCheck.check2DPadding(padding);

	let width = Math.floor((input[0] - filter[0] + 2 * padding[0]) / stride[0] + 1);
	let height = Math.floor((input[1] - filter[1] + 2 * padding[1]) / stride[1] + 1);
 ...
 */
function analyzePool(input, filter, stride, padding, options) {
	options = options || {};
	padding = padding || [0, 0];
	...
	for (var i = 0; i < networkJson.length; i++) {
		...
		if (layer.type === 'conv') {
			...
			if (options.tensorflow)
				stat = tfAnalyzer.analyzeConvTF(input, layer.filter, layer.stride, layer.padding, options);
			else
				stat = analyzeConv(input, layer.filter, layer.stride, layer.padding, options);
		} else if (layer.type === 'pool') {
			// If tensorflow flag is set, user TF analyzer instead
			if (options.tensorflow)
				stat = tfAnalyzer.analyzePoolTF(input, layer.filter, layer.stride, layer.padding, options);
			else
				stat = analyzePool(input, layer.filter, layer.stride, layer.padding, options);
